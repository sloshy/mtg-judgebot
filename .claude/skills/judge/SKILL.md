...
from the repo root (it reads `.env` there: `DATABASE_URL` is required; `ANTHROPIC_API_KEY`
only for `judge`; `VOYAGE_API_KEY` optional). Logs go to stderr, JSON to stdout. Flags can
