...
several cards (the Tron lands, "karn", "emrakul", "sheoldred") are deliberately left out:
...
and drops the zero, and the loader rejects it. `judge-eval recall` checks retrieval against
it for free; `judge-eval answer` runs the whole pipeline (about $2.50 for the set); `rescore`
re-grades stored runs after an edit. Extend it when adding capability, and re-verify rule
ids on each CR release.
