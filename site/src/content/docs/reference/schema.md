...
One Postgres database with the `vector` (pgvector) and `pg_trgm` extensions. Migrations
live in `crates/bot/migrations/` and are embedded in the binaries; `bot` and `api` apply
pending ones at startup, `judge-ingest migrate` is the explicit form. Every SQL query is
...
| Table | Contents |
| --- | --- |
| `rules` | Two granularities: rule-level rows (`702.19`, body includes the lettered sub-rules and examples; these carry embeddings and feed retrieval) and leaf rows (`702.19b`, `parent_id` set; citation targets). A `cr_version` per load. |
...
renumbering pass matches old and new rules by body with every rule id masked and rewrites
...
| Table | Contents |
| --- | --- |
| `calls` | Every answered question: thread id, question, answer, category, source, confidence, the citations, the ids of the context it was answered from (including a fingerprint of each context card's Oracle text, computed at persist time, so an erratum retires calls *about* a card even when they cited only the CR), `cr_version`, an embedding, and `retired_at`/`retired_reason` when its citations stopped holding. `session_id` is set for calls persisted through an agent session. |
...
advisory lock, so a switch waits for in-flight writes and a write after it sees the new
state.
