...
Every judgebot is run by the community that uses it: your Discord application, your
model key, your spend cap. A complete instance is three containers from one compose
file: `db` (Postgres with pgvector), `api` (the web page and HTTP API) and `bot`
(Discord). A fourth, `refresh`, runs nightly from cron to keep cards and rules current,
and an optional `cloudflared` publishes the API without an open port. You can stop after
`api` and never touch Discord; the bot is the last thing to add.
...
4. `docker compose up -d api`. Open <http://localhost:8787> and ask a question; this is the
   whole pipeline, and a good place to check the model setup before Discord is involved.
...
5. [Create the Discord app](../../self-hosting/discord-app/), then `docker compose up -d
   bot`, then once `cargo run --release -p judge-ingest -- emoji` so answers show mana
