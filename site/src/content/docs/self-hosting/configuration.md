...
| `JUDGE_MAX_USD` | `5` | bot, api, eval, agent | Spend cap per process across every provider. The cap reserves each call's worst case first, so values under about $0.45 refuse synthesis. Once the headroom left is smaller than a call's worst case, questions are refused. `JUDGE_BUDGET_PERIOD` says what the cap covers. |
| `JUDGE_BUDGET_PERIOD` | `process` | bot, api | What `JUDGE_MAX_USD` caps. `process`: each of `bot` and `api` for its lifetime, held in memory, so a restart counts from zero and a compose deployment can spend twice the cap. `day` or `month`: the current UTC day or month, shared by `bot` and `api` through the `spend_days` table and kept across restarts. The processes sync every 10 seconds, so together they can overshoot by what they spend in that time. `judge-cli` and `judge-eval` always cap per process. |
...
| `JUDGE_USER_LIMIT`, `JUDGE_USER_WINDOW_SECS` | `6`, `600` | bot | `/judge` questions per Discord member per window. A "busy" reply does not count. `0` turns the limit off. Held in memory. `/card` and `/rule` are not limited. |
