...
| `JUDGE_MAX_USD` | `5` | bot, api, eval, agent | Spend cap per process across every provider. The cap reserves each call's worst case first, so values under about $0.45 refuse synthesis. |
