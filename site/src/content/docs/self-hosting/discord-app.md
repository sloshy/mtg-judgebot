...
channel permissions**, because it receives only its own commands and button presses
...
3. **Public Bot** can be off. It controls whether *other* people can use your install link.
   A bot you run for your own servers has no reason to allow that.
...
The running bot registers its slash commands, per application. It registers them either in
one server (instant) or globally (every server the bot is in, up to an hour to appear).
