    ...
    fn decode(&self, reply: &Reply) -> Result<ChatResponse, LlmError> {
        if !reply.status.is_success() {
            let (kind, message) = match serde_json::from_slice::<ApiErrorBody>(&reply.body) {
                Ok(b) => (
                    b.error.kind.unwrap_or_else(|| "unknown".to_owned()),
                    b.error.message,
                ),
                Err(_) => (
                    "unknown".to_owned(),
        ...
        from_wire(&parsed, &self.model).map_err(|source| LlmError::Decode {
            source,
            billed: usage_of(&reply.body),
        })
    }
...
impl Backend for OpenAi {
    ...
    async fn complete(&self, req: &ChatRequest) -> Result<ChatResponse, LlmError> {
        if req.effort.is_some()
            && !self.dialect.reasoning_effort
            && !self.warned_effort.swap(true, Ordering::Relaxed)
        {
...
mod tests {
    use super::*;
    use judge_llm::{
        ChatModel as _, Effort, Metered, Price, SpendMeter, TextBlock, ToolChoice, Turn,
    };
    use serde_json::json;
    use wiremock::{
    ...
    async fn bearer_auth_url_and_usage_reach_the_meter() -> Result<(), LlmError> {
        ...
        assert!(
            (metered.meter().spent_usd() - expected).abs() < 1e-9,
            "{}",
            metered.meter().spent_usd()
        );
    ...
    async fn azure_header_and_query_string() -> Result<(), LlmError> {
        ...
        assert!(
            reqs.first()
                .is_some_and(|r| r.headers.get("authorization").is_none())
        );
        Ok(())
    }
    ...
    async fn api_errors_carry_the_message_and_undecodable_bodies_are_billed() -> Result<(), LlmError>
    {
        ...
        let r = client.complete(&req()).await;
        assert!(
            matches!(&r, Err(LlmError::Api { kind, message, .. }) if kind == "unknown" && message == "model not found"),
            "{r:?}"
        );
        let reqs = server.received_requests().await.unwrap_or_default();
        assert!(reqs.first().is_some_and(
            |r| r.headers.get("authorization").is_none() && r.headers.get("api-key").is_none()
        ));
        ...
        Mock::given(method("POST"))
            ...
            .respond_with(ResponseTemplate::new(200).set_body_json(
                json!({"id": "x", "usage": {"prompt_tokens": 1_000_000, "completion_tokens": 0}}),
            ))
            .mount(&server)
            .await;
        let client = OpenAi::new(
        ...
        assert!(
            (metered.meter().spent_usd() - 5.0).abs() < 1e-6,
            "{}",
            metered.meter().spent_usd()
        );
        Ok(())
    ...
    #[tokio::test]
    async fn effort_is_dropped_with_one_warning_when_the_dialect_cannot_send_it()
    -> Result<(), LlmError> {
        let server = MockServer::start().await;
        Mock::given(method("POST"))
            .and(path("/v1/chat/completions"))
    ...
    fn debug_redacts_the_key() -> Result<(), LlmError> {
        let c = OpenAi::new(
            "http://x/v1",
            Auth::Bearer("sk-super-secret".into()),
            "m",
            Dialect::default(),
        )?;
        let s = format!("{c:?}");
        assert!(
            !s.contains("super-secret") && s.contains("<redacted>"),
            "{s}"
        );
        assert_eq!(c.model(), "m");
