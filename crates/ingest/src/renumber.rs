    ...
    loop {
        let drop: Vec<RuleId> = map
            ...
            .filter(|(from, to)| {
                !consistent(
                    &map,
                    old_by_id.get(from).copied(),
                    new_by_id.get(to).copied(),
                )
            })
...
fn candidates(old: &[StoredRule], new: &[RuleChunk]) -> BTreeMap<RuleId, RuleId> {
    ...
    let old_rules = unique_by_masked(
        old.iter()
            .filter(|r| r.parent_id.is_none())
            .map(|r| (&r.id, r.body.as_str())),
    );
    let new_rules = unique_by_masked(
        new.iter()
            .filter(|r| r.parent_id.is_none())
            .map(|r| (&r.id, r.body.as_str())),
    );
    ...
    let old_first_lines: HashMap<&RuleId, String> = old
        .iter()
        .filter(|r| r.parent_id.is_none())
        .map(|r| (&r.id, masked(first_line(&r.body))))
        .collect();
    let new_first_lines: HashMap<&RuleId, String> = new
        .iter()
        .filter(|r| r.parent_id.is_none())
        .map(|r| (&r.id, masked(first_line(&r.body))))
        .collect();
    for (old_parent, old_leaves) in &old_leaves_by_parent {
        let new_parent = match map.get(*old_parent) {
            Some(p) => p,
            None if old_first_lines.get(old_parent) == new_first_lines.get(old_parent) => {
                old_parent
            }
            None => continue,
        };
        let Some(new_leaves) = new_leaves_by_parent.get(new_parent) else {
            continue;
        };
...
pub fn rewrite_ids(text: &str, map: &BTreeMap<RuleId, RuleId>) -> Option<String> {
    ...
    let out = RULE_REF.replace_all(text, |caps: &regex::Captures<'_>| {
        ...
        match RuleId::try_new(found.to_owned())
            .ok()
...
pub fn rewrite_call(
    ...
    if let Some(items) = citations.as_array_mut() {
        for c in items.iter_mut() {
            let Some(obj) = c.as_object_mut() else {
                continue;
            };
            let kind = obj
                .get("kind")
            ...
            if kind == "rule"
                ...
                && let Some(to) = RuleId::try_new(id.to_owned())
                    .ok()
                    .and_then(|id| map.get(&id))
            {
                obj.insert(
                    "id".to_owned(),
                    serde_json::Value::String(to.as_ref().to_owned()),
                );
                changed = true;
            }
...
mod tests {
    ...
    fn inserted_rule_shifts_the_rest_and_cross_references_are_seen_through() {
        ...
        assert_eq!(
            ...
            m(&[("702.20", "702.21"), ("702.20a", "702.21a")])
        );
    }
    ...
    fn a_redirected_cross_reference_is_not_a_renumbering() {
        ...
        let old = [
            stored("702.20", None, "702.20. Vigilance. See rule 702.19."),
            stored("702.19", None, "702.19. Trample."),
        ];
        let new = [
            chunk("702.19", None, "702.19. Trample."),
...
724.2b Also new.",
    ...
    fn examples_take_part_in_the_check() {
        ...
        assert_eq!(
            renumber_map(&[old.clone()], &[filler.clone(), new.clone()]),
            m(&[("702.20", "702.21")])
        );
    ...
    fn rewrite_is_single_pass_and_bounded_by_word_edges() {
        ...
        assert_eq!(
            rewrite_ids(
                "See 702.19 and 702.20; also 702.19b, not 1702.19 or 702.190.",
                &map
            )
            .as_deref(),
    ...
    fn rewrite_call_touches_rule_and_prior_call_quotes_only() -> anyhow::Result<()> {
        ...
        let (c, a) = rewrite_call(&citations, "Rule 702.19b says so.", &map)
            .ok_or_else(|| anyhow::anyhow!("expected a change"))?;
        assert_eq!(a, "Rule 702.20b says so.");
        assert_eq!(at(&c, 0, "id"), Some(&serde_json::json!("702.20b")));
        assert_eq!(
            at(&c, 0, "quote"),
            Some(&serde_json::json!(
                "702.20b The controller of an attacking creature"
            ))
        );
        assert_eq!(
            at(&c, 1, "quote"),
            Some(&serde_json::json!("see rule 702.19b")),
            "ruling text was not renumbered"
        );
        assert_eq!(
            at(&c, 2, "quote"),
            Some(&serde_json::json!("Trample (702.19b)")),
            "Oracle text was not renumbered"
        );
        assert_eq!(
            at(&c, 3, "quote"),
            Some(&serde_json::json!("Under 702.20b you assign lethal")),
            "answers are rewritten by the same map"
        );
        assert_eq!(
            c.get(4),
            citations.get(4),
            "an unreadable element is carried through"
        );
        assert_eq!(
            rewrite_call(&citations, "nothing here", &m(&[("100.1", "100.2")])),
            None
        );
        Ok(())
    }
