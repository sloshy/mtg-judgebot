...
use judge_core::{
    CallStore, Category, CategoryGuess, Confidence, Extraction, JudgeError, MatchedVia, Question, Resolution,
    Resolver, Retriever, RuleId, Score, Source, Verdict,
};
use sqlx::PgPool;
use uuid::Uuid;
...
async fn seed(pool: &PgPool) -> anyhow::Result<()> {
    let cards: [(Uuid, &str, &str, &[FaceSeed]); 9] = [
        (BOB, "Dark Confidant", "normal", &[(0, "Dark Confidant", "At the beginning of your upkeep, reveal the top card of your library and put that card into your hand. You lose life equal to its mana value.")]),
        (URZA_MINE, "Urza's Mine", "normal", &[(0, "Urza's Mine", "{T}: Add {C}.")]),
        (URZA_TOWER, "Urza's Tower", "normal", &[(0, "Urza's Tower", "{T}: Add {C}.")]),
        (URZA_PLANT, "Urza's Power Plant", "normal", &[(0, "Urza's Power Plant", "{T}: Add {C}.")]),
        (BONECRUSHER, "Bonecrusher Giant // Stomp", "adventure", &[
            (0, "Bonecrusher Giant", "Whenever this creature becomes the target of a spell, it deals 2 damage to that spell's controller."),
            (1, "Stomp", "Damage can't be prevented this turn. Stomp deals 2 damage to any target."),
        ]),
        (BALLISTA, "Walking Ballista", "normal", &[(0, "Walking Ballista", "This creature enters with X +1/+1 counters on it.")]),
        (BOLT, "Lightning Bolt", "normal", &[(0, "Lightning Bolt", "Lightning Bolt deals 3 damage to any target.")]),
        (EMERITUS, "Emeritus of Conflict // Lightning Bolt", "prepare", &[
            (0, "Emeritus of Conflict", "Prepare {1}{R}"),
            (1, "Lightning Bolt", "Lightning Bolt deals 3 damage to any target."),
        ]),
        (LEOTAU, "Grizzled Leotau", "normal", &[(0, "Grizzled Leotau", "")]),
    ];
    for (id, name, layout, faces) in cards {
        sqlx::query("INSERT INTO cards (oracle_id, name, layout) VALUES ($1, $2, $3)")
            .bind(id).bind(name).bind(layout).execute(pool).await?;
    ...
    for id in [URZA_MINE, URZA_TOWER] {
        sqlx::query("INSERT INTO printed_names (printed_name, oracle_id) VALUES ('Urza Land', $1)").bind(id).execute(pool).await?;
    }
    sqlx::query("INSERT INTO card_aliases (alias, oracle_id) VALUES ('bob', $1)").bind(BOB).execute(pool).await?;
    sqlx::query("INSERT INTO printed_names (printed_name, oracle_id) VALUES ('Urzas Mine (old)', $1)")
        .bind(URZA_MINE).execute(pool).await?;
    sqlx::query("INSERT INTO card_notes (oracle_id, note) VALUES ($1, 'Stomp is an Adventure: see 715.')")
    ...
    let rules: [(&str, Option<&str>, &str, &str, &str); 8] = [
        ("613", None, "613", "Interaction of Continuous Effects", "Section header."),
        ("613.1", None, "613", "", "The values of an object's characteristics are determined by starting with the actual object. 613.1d Layer 4: Type-changing effects are applied."),
        ("613.1d", Some("613.1"), "613", "", "Layer 4: Type-changing effects are applied. This includes effects that change an object's card type, subtype, and/or supertype."),
        ("613.2", None, "613", "", "Within layers 1-6, apply effects from characteristic-defining abilities first."),
        ("614.1", None, "614", "", "Some continuous effects are replacement effects."),
        ("702.19", None, "702", "Trample", "702.19a Trample is a static ability. 702.19b The controller of an attacking creature with trample first assigns the combat damage to the creature(s) blocking it. Once all those blocking creatures are assigned lethal damage, any remaining damage is assigned as its controller chooses."),
        ("702.2", None, "702", "Deathtouch", "702.2b Any nonzero amount of combat damage assigned by a source with deathtouch to a creature is considered to be lethal damage."),
        ("903.4", None, "903", "", "The Commander variant uses color identity to determine what cards can be in a deck."),
    ];
...
async fn alias_hit(pool: PgPool) -> anyhow::Result<()> {
    ...
    assert_eq!(resolved_name(&r), Some(("Dark Confidant", MatchedVia::Alias)));
    Ok(())
}
...
async fn bracket_hit_on_card_and_face_names(pool: PgPool) -> anyhow::Result<()> {
    ...
    assert_eq!(resolved_name(&r), Some(("Dark Confidant", MatchedVia::Bracket)));
    let r = resolver.resolve("[[Stomp]]").await?;
    assert_eq!(resolved_name(&r), Some(("Bonecrusher Giant // Stomp", MatchedVia::Bracket)));
...
async fn printed_and_exact_rungs(pool: PgPool) -> anyhow::Result<()> {
    ...
    assert_eq!(resolved_name(&r), Some(("Urza's Mine", MatchedVia::PrintedName)));
    // The current name reports `Exact` even though it is also a printed name.
    let r = resolver.resolve("walking ballista").await?;
    assert_eq!(resolved_name(&r), Some(("Walking Ballista", MatchedVia::Exact)));
    sqlx::query("DELETE FROM printed_names").execute(&pool).await?;
    let r = resolver.resolve("walking ballista").await?;
    assert_eq!(resolved_name(&r), Some(("Walking Ballista", MatchedVia::Exact)));
    Ok(())
}
...
#[sqlx::test(migrations = "./migrations")]
async fn full_name_beats_face_name_and_shared_names_stay_ambiguous(pool: PgPool) -> anyhow::Result<()> {
    seed(&pool).await?;
    let resolver = PgResolver::new(pool);
    // "Lightning Bolt" is a card and a face of "Emeritus of Conflict // Lightning Bolt".
    let r = resolver.resolve("lightning bolt").await?;
    assert_eq!(resolved_name(&r), Some(("Lightning Bolt", MatchedVia::Exact)));
    let r = resolver.resolve("[[Lightning Bolt]]").await?;
    assert_eq!(resolved_name(&r), Some(("Lightning Bolt", MatchedVia::Bracket)));
    let r = resolver.resolve("Emeritus of Conflict").await?;
    assert_eq!(resolved_name(&r), Some(("Emeritus of Conflict // Lightning Bolt", MatchedVia::Exact)));
...
async fn fuzzy_ignores_a_span_buried_inside_a_word(pool: PgPool) -> anyhow::Result<()> {
    ...
    assert!(matches!(resolver.resolve("led").await?, Resolution::NotFound { .. }));
    Ok(())
}
...
async fn fuzzy_resolves_a_typo(pool: PgPool) -> anyhow::Result<()> {
    ...
    assert_eq!(resolved_name(&r), Some(("Walking Ballista", MatchedVia::Fuzzy)));
    Ok(())
}
...
async fn not_found_for_gibberish(pool: PgPool) -> anyhow::Result<()> {
    seed(&pool).await?;
    let resolver = PgResolver::new(pool);
    assert!(matches!(resolver.resolve("xqzvv plorth").await?, Resolution::NotFound { query } if query == "xqzvv plorth"));
    assert!(matches!(resolver.resolve("  ").await?, Resolution::NotFound { .. }));
    Ok(())
}
...
fn extraction(categories: &[Category], concepts: &[&str]) -> Extraction {
    Extraction {
        ...
        categories: categories.iter().map(|c| CategoryGuess { category: *c, confidence: Confidence::High }).collect(),
        source: Source::Cr,
    }
}

fn question(text: &str) -> Question {
    Question { thread_id: "t1".into(), text: text.into() }
}
...
async fn retrieve_unions_category_map_and_full_text(pool: PgPool) -> anyhow::Result<()> {
    seed(&pool).await?;
    let resolver = PgResolver::new(pool.clone());
    let Resolution::Resolved { card, .. } = resolver.resolve("[[Stomp]]").await? else { anyhow::bail!("no card") };
    let retriever = PgRetriever::new(pool);
    let ctx = retriever
        .retrieve(&question("does trample need lethal damage?"), &[card], &extraction(&[Category::Layers], &["trample lethal damage"]))
        .await?;
    ...
    assert_eq!(ctx.cr_version().map(|v| -> &str { v.as_ref() }), Some("20260819"));

    assert_eq!(ctx.rulings.len(), 2);
    assert_eq!(ctx.rulings.first().map(|r| r.published_at.as_str()), Some("2019-10-04"));
...
async fn lookup_rules_expands_subsections(pool: PgPool) -> anyhow::Result<()> {
    ...
    let ids = |s: &[&str]| -> Result<Vec<RuleId>, JudgeError> {
        s.iter().map(|id| RuleId::try_new((*id).to_owned()).map_err(anyhow::Error::from).map_err(JudgeError::from)).collect()
    };
    ...
    assert_eq!(chunks.last().and_then(|c| c.parent_id.as_ref()).map(|p| -> &str { p.as_ref() }), Some("613.1"));
...
async fn persist_then_prior_calls_and_rate_upserts(pool: PgPool) -> anyhow::Result<()> {
    ...
    let verdict = Verdict::new("Layer 4 is types.".into(), Confidence::High, vec![], Category::Layers, Source::Cr).validate(&ctx)?;
    let call = store.persist(&q, &verdict, &ctx).await?;

    let ctx2 = retriever.retrieve(&q, &[], &extraction(&[Category::Layers], &[])).await?;
    assert_eq!(ctx2.prior.len(), 1);
    let prior = ctx2.prior.first().ok_or_else(|| anyhow::anyhow!("no prior"))?;
    assert_eq!(prior.id, call);
    assert_eq!(prior.rating_count, 0);
    assert!((prior.rating - 2.0).abs() < 1e-6);
    // Other categories do not see it.
    let other = retriever.retrieve(&q, &[], &extraction(&[Category::Combat], &[])).await?;
    assert!(other.prior.is_empty());
    // A question about a card the call did not involve does not see it either.
    let Resolution::Resolved { card: bob, .. } = PgResolver::new(pool.clone()).resolve("bob").await? else { anyhow::bail!("no bob") };
    let unrelated = retriever.retrieve(&q, std::slice::from_ref(&bob), &extraction(&[Category::Layers], &[])).await?;
    assert!(unrelated.prior.is_empty());
    let ctx_bob = retriever.retrieve(&q, std::slice::from_ref(&bob), &extraction(&[Category::Layers], &[])).await?;
    let v2 = Verdict::new("Bob answer.".into(), Confidence::High, vec![], Category::Layers, Source::Cr).validate(&ctx_bob)?;
    let call_bob = store.persist(&q, &v2, &ctx_bob).await?;
    let with_bob = retriever.retrieve(&q, std::slice::from_ref(&bob), &extraction(&[Category::Layers], &[])).await?;
    assert_eq!(with_bob.prior.iter().map(|p| p.id).collect::<Vec<_>>(), vec![call_bob]);
    sqlx::query("DELETE FROM calls WHERE id = $1").bind(call_bob.into_inner()).execute(&pool).await?;

    // rate() upserts on (call_id, user_id).
    store.rate(call, "u1", Score::Correct, false).await?;
    store.rate(call, "u1", Score::Incorrect, true).await?;
    let (n, score, judge): (i64, i16, bool) =
        sqlx::query_as("SELECT count(*), min(score), bool_or(is_judge) FROM ratings WHERE call_id = $1")
            .bind(call.into_inner()).fetch_one(&pool).await?;
    assert_eq!((n, score, judge), (1, 1, true));
    let ctx3 = retriever.retrieve(&q, &[], &extraction(&[Category::Layers], &[])).await?;
    let prior = ctx3.prior.first().ok_or_else(|| anyhow::anyhow!("no prior"))?;
    assert_eq!(prior.rating_count, 1);
    assert!((prior.rating - 1.0).abs() < 1e-6, "judge score overrides: {}", prior.rating);
    ...
    let ctx4 = retriever.retrieve(&q, &[], &extraction(&[Category::Layers], &[])).await?;
    ...
    let ctx5 = retriever.retrieve(&q, &[], &extraction(&[Category::Layers], &[])).await?;
