...
use judge_llm::{
    AssistantTurn, Billed, CacheHint, ChatRequest, ChatResponse, Effort, LlmError, Refusal, RefusalFallback, Stop,
...
fn system_block(b: &TextBlock) -> SystemBlock {
    SystemBlock::Text { text: b.text.clone(), cache_control: b.cache.map(cache_control) }
}
...
fn message(turn: &Turn) -> Result<Message, LlmError> {
    Ok(match turn {
        Turn::User(blocks) => Message {
            ...
            content: blocks
                .iter()
                .map(|b| ContentBlock::Text { text: b.text.clone(), cache_control: b.cache.map(cache_control) })
                .collect(),
        },
        Turn::Assistant(AssistantTurn { backend, raw }) => {
            ...
            let content: Vec<ContentBlock> = serde_json::from_value(raw.clone())
                .map_err(|e| LlmError::Request(format!("assistant turn is not Messages API content: {e}")))?;
        ...
        Turn::ToolResults(results) => Message {
            ...
            content: results
                .iter()
                .map(|r| ContentBlock::ToolResult { tool_use_id: r.call_id.clone(), content: r.content.clone(), is_error: r.is_error })
                .collect(),
        },
    })
}
...
fn tool_choice(choice: ToolChoice) -> wire::ToolChoice {
    match choice {
        ...
        ToolChoice::Auto { parallel: true } => wire::ToolChoice::Auto { disable_parallel_tool_use: None },
...
fn fallbacks(f: &RefusalFallback) -> Fallbacks {
    match f {
        ...
        RefusalFallback::Models(models) => Fallbacks::Models(models.iter().map(|m| FallbackModel { model: m.clone() }).collect()),
    }
}
...
pub fn from_wire(resp: &MessagesResponse) -> Result<ChatResponse, serde_json::Error> {
    let stop = match resp.stop_reason {
        ...
        Some(StopReason::Refusal) => Stop::Refusal(
            resp.stop_details
                .as_ref()
                .map(|d| Refusal { category: d.category.clone(), explanation: d.explanation.clone() })
                .unwrap_or_default(),
        ),
    ...
    Ok(ChatResponse {
        ...
        tool_calls: resp
            .tool_uses()
            .map(|(id, name, input)| ToolCall { id: id.to_owned(), name: name.to_owned(), input: input.clone() })
            .collect(),
        stop,
        usage: usage(&resp.usage),
        model: resp.model.clone(),
        assistant: AssistantTurn { backend: BACKEND, raw: serde_json::to_value(&resp.content)? },
    })
}
...
mod tests {
    ...
    fn every_knob_maps_to_its_field() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(at(&v, "/tools/0/input_schema/additionalProperties"), &Value::Bool(false), "the subset transform ran");
        assert!(!at(&v, "/tools/0/input_schema").to_string().contains("pattern"), "RuleId's regex is stripped from the tool schema");
        assert_eq!(at(&v, "/tool_choice"), &json!({"type": "auto", "disable_parallel_tool_use": true}));
        ...
        assert!(!at(&v, "/output_config/format/schema").to_string().contains("oneOf"), "the subset transform ran");
        ...
        assert_eq!(at(&serde_json::to_value(to_wire("m", &none)?)?, "/tool_choice"), &json!({"type": "none"}));
        let parallel = ChatRequest { tool_choice: ToolChoice::Auto { parallel: true }, ..r.clone() };
        assert_eq!(at(&serde_json::to_value(to_wire("m", &parallel)?)?, "/tool_choice"), &json!({"type": "auto"}));
        let models = ChatRequest { fallbacks: Some(RefusalFallback::Models(vec!["claude-opus-4-8".into()])), ..r };
        assert_eq!(at(&serde_json::to_value(to_wire("m", &models)?)?, "/fallbacks"), &json!([{"model": "claude-opus-4-8"}]));
        Ok(())
    }
    ...
    fn assistant_turns_replay_verbatim_and_foreign_ones_are_refused() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let r = ChatRequest {
            turns: vec![
                ...
                Turn::Assistant(AssistantTurn { backend: BACKEND, raw: content.clone() }),
                Turn::ToolResults(vec![ToolResult { call_id: "tu_1".into(), content: "rules".into(), is_error: false }]),
            ],
            ..req()
        };
        let v = serde_json::to_value(to_wire("m", &r)?)?;
        assert_eq!(at(&v, "/messages/1"), &json!({"role": "assistant", "content": content}));
        assert_eq!(at(&v, "/messages/2"), &json!({"role": "user", "content": [{"type": "tool_result", "tool_use_id": "tu_1", "content": "rules"}]}));

        let foreign = ChatRequest { turns: vec![Turn::Assistant(AssistantTurn { backend: "openai", raw: json!({}) })], ..req() };
        assert!(matches!(to_wire("m", &foreign), Err(LlmError::ForeignTurn { expected: BACKEND, found: "openai" })));
        let garbage = ChatRequest { turns: vec![Turn::Assistant(AssistantTurn { backend: BACKEND, raw: json!(7) })], ..req() };
    ...
    fn responses_read_as_text_calls_stop_and_usage() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(c.tool_calls, [ToolCall { id: "tu_1".into(), name: "lookup_rules".into(), input: json!({"ids": ["613"]}) }]);
        assert_eq!(c.stop, Stop::ToolUse);
        assert_eq!(c.usage, Usage { input: 1, output: 2, cache_read: 3, cache_write: 0 });
        assert_eq!(c.model, "claude-opus-5");
        assert_eq!(c.assistant.backend, BACKEND);
        assert_eq!(c.assistant.raw, content, "the whole content array, thinking included");
        ...
        for (wire, stop) in [
            ...
            ("refusal", Stop::Refusal(Refusal { category: Some("cyber".into()), explanation: None })),
        ] {
    ...
    fn usage_of_reads_a_body_that_is_not_a_response() {
        let b = usage_of(br#"{"id": "m", "model": "x", "usage": {"input_tokens": 5, "output_tokens": 0}}"#);
        assert_eq!(b, Some(Billed { model: Some("x".into()), usage: Usage { input: 5, ..Usage::default() } }));
