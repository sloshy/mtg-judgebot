...
use judge_core::{Category, CategoryGuess, Citation, Confidence, Extraction, RuleId, Source, Verdict};
use sqlx::PgPool;
...
use crate::{
    ...
    ops::{
        BeginInput, ExtractionReply, IdsInput, JudgeInput, JudgeReply, LookupInput, NameInput, SearchInput, SessionInput,
        VerdictInput, VerdictReply,
    },
};
...
async fn seed(pool: &PgPool) -> anyhow::Result<()> {
    sqlx::query("INSERT INTO cards (oracle_id, name, layout) VALUES ($1, 'Lightning Bolt', 'normal')")
        .bind(BOLT)
        .execute(pool)
        .await?;
    sqlx::query(
    ...
    for (id, parent, sub, heading, body) in [
        ("702", None, "702", "Keyword Abilities", "Keyword abilities."),
    ...
    sqlx::query("INSERT INTO categories (id, label, subsections) VALUES ('combat', 'Combat', '{702.15}')")
        .execute(pool)
        .await?;
    Ok(())
}
...
async fn a_session_through_the_ops_layer(pool: PgPool) -> anyhow::Result<()> {
    ...
    assert!(matches!(t.judge(JudgeInput { question: "q?".into(), thread: None, pins: vec![] }).await?, JudgeReply::Unavailable { .. }));

    let begun = t.begin_session(BeginInput { question: "does bolt's damage gain me life?".into(), thread: None }).await?;
    let ExtractionReply::Ready { prompt } = t.submit_extraction(crate::ops::ExtractionInput { session: begun.session, extraction: extraction() }).await? else {
    ...
    let status = t.session_status(SessionInput { session: begun.session }).await?;
    assert_eq!((status.stage.as_str(), status.attempts, status.lookup_available), ("awaiting_verdict", Some(1), false));
    assert!(t.lookup_rules(LookupInput { session: begun.session, ids: vec![rid("120")?] }).await.is_err(), "forfeited");
    ...
    let VerdictReply::Accepted { answer, call, persist_error } =
        t.submit_verdict(VerdictInput { session: begun.session, verdict: good, persist: true }).await?
    else {
        anyhow::bail!("expected accepted");
    };
    assert_eq!(persist_error, None);
    assert_eq!(answer.citations.first().map(|c| c.label.as_str()), Some("702.15"));
    let again = t.persist_session(SessionInput { session: begun.session }).await?;
    ...
    assert_eq!(n, 1);
    let status = t.session_status(SessionInput { session: begun.session }).await?;
    assert_eq!((status.stage.as_str(), status.attempts, status.outcome.as_deref()), ("closed", None, Some("answered")));

    // The follow-up in the same thread sees the answer as history.
    let next = t.begin_session(BeginInput { question: "and with lifelink?".into(), thread: Some(begun.thread) }).await?;
    assert!(next.extraction.user.contains("does bolt's damage gain me life?"));
    Ok(())
}
...
async fn lookups_are_bounded_and_the_judge_quota_counts(pool: PgPool) -> anyhow::Result<()> {
    seed(&pool).await?;
    let t = toolbox(pool, Some(Quota { limit: 1, window: std::time::Duration::from_hours(1) }));
    assert!(t.resolve_card(NameInput { name: "x".repeat(300) }).await.is_err());
    assert!(t.resolve_card(NameInput { name: " ".into() }).await.is_err());
    let many: Vec<RuleId> = (0..11).map(|i| rid(&format!("{}", 100 + i))).collect::<anyhow::Result<_>>()?;
    assert!(t.get_rules(IdsInput { ids: many }).await.is_err());
    assert!(t.get_rules(IdsInput { ids: vec![] }).await.is_err());
    assert_eq!(t.get_rules(IdsInput { ids: vec![rid("702")?] }).await?.rules.len(), 1, "a subsection expands to its rules");
    assert!(t.search_rules(SearchInput { query: "  ".into(), limit: None }).await.is_err());
    let hits = t.search_rules(SearchInput { query: "lifelink".into(), limit: Some(1000) }).await?;
    ...
    let q = || JudgeInput { question: "q?".into(), thread: None, pins: vec![] };
    assert!(matches!(t.judge(q()).await?, JudgeReply::Unavailable { .. }));
    assert!(matches!(t.judge(q()).await?, JudgeReply::Unavailable { .. }));
